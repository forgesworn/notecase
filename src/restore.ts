...
import {HashLookupUnsupportedError} from './lnurlcash-errors.js'
import {fetchNoteInfoByHash, fetchNoteInfoDisclosingK1} from './lnurlcash-network.js'
import {buildNoteUrl, hashK1} from '@lnurlcash/kit'
import {deriveNoteRoot, deriveNoteSecret} from './legacy-secrets.js'
import {cashSecretAt, deriveCashDomainNode, deriveCashRoot} from './cash.js'
import type {LnurlcashOptions} from './lnurlcash-network.js'
...
export type RestoreOptions = {
  ...
  // service may prune spent-note identifiers, and a gap larger than `gap`
  // still prevents a wallet rediscovering its own position from the service
  // alone. The current reference mint does distinguish retained spent notes.
  start?: number
...
export const restoreNotes = async (
  ...
  const walked = await restoreSchemes(
    ...
    {gap, ...(probeK1 === undefined ? {} : {probeK1}), allowSecretDisclosure},
    options
  )
...
const restoreSchemes = async (
  ...
  for (const scheme of schemes) {
    ...
    if (outcome.found.length > 0 || outcome.unresolved.length > 0 || outcome.lastUsed !== null) {
      hashLookupsConfirmed = true
    }
    byHash.push(outcome)
  }
  ...
  for (const scheme of schemes) {
    bySecret.push(
      await walk(scheme, gap, k1 =>
        fetchNoteInfoDisclosingK1(buildNoteUrl(baseUrl, k1), options)
      )
    )
  }
